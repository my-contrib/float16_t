...
#include "half-private/helpers.hh"
...
constexpr inline std::uint16_t half_add(std::uint16_t x,
  ...
  const std::uint16_t c_s_diff =
    half_private::_uint16_sels(is_c_m_ab_pos_msb, a_s, b_s);
  const std::uint16_t c_s =
    half_private::_uint16_sels(is_diff_sign_msb, c_s_diff, a_s);
  const std::uint16_t c_m_smag_diff_nlz =
    half_private::_uint16_cntlz(c_m_smag_diff);
  ...
  const std::uint16_t c_m_diff_ab_norm = half_private::_uint16_sels(
    is_diff_denorm_msb, c_m_diff_denorm, c_m_diff_norm);
  const std::uint16_t c_e_diff_ab_norm = (c_e_diff_norm & ~is_diff_denorm);
  const std::uint16_t c_m_diff = half_private::_uint16_sels(
    is_a_or_b_norm_msb, c_m_diff_ab_norm, c_m_smag_diff);
  const std::uint16_t c_e_diff = half_private::_uint16_sels(
    is_a_or_b_norm_msb, c_e_diff_ab_norm, a_e_biased);
  const std::uint16_t is_diff_eqz_msb = (c_m_diff - 1);
  const std::uint16_t is_diff_exactly_zero_msb =
    (is_diff_sign_msb & is_diff_eqz_msb);
  const std::uint16_t is_diff_exactly_zero =
    (((int16_t)is_diff_exactly_zero_msb) >> 15);
  ...
  const std::uint16_t c_e_hidden_offset =
    ((c_m_added & h_m_grs_carry) >> h_m_grs_carry_pos);
...
constexpr inline std::uint16_t half_mul(std::uint16_t x,
                                        std::uint16_t y) noexcept {
  const std::uint32_t one = (0x00000001);
  const std::uint32_t h_s_mask = (0x00008000);
  const std::uint32_t h_e_mask = (0x00007c00);
  ...
  const std::uint32_t c_m_denorm_rounded =
    (c_m_denorm_biased + c_m_denorm_round_amount);
  const std::uint32_t c_m_denorm_inplace =
    (c_m_denorm_rounded >> h_m_bit_count);
  const std::uint32_t c_m_denorm_unbiased =
    (c_m_denorm_inplace >> c_e_denorm_unbias_e);
  ...
  const std::uint32_t is_c_e_unbiased_underflow =
    (((std::int32_t)c_e_amount_unbiased) >> 31);
  ...
  const std::uint32_t c_m_underflow_added = half_private::_uint32_selb(
    is_c_e_unbiased_underflow, c_m_underflow, c_m_normal);
  const std::uint32_t is_mul_overflow_test =
    (c_e_underflow_added & m_round_overflow_bit);
  ...
  const std::uint32_t c_m_norm_hidden_bit =
    (c_m_norm_radix_corrected & m_hidden_bit);
  const std::uint32_t is_c_m_norm_no_hidden_msb = (c_m_norm_hidden_bit - 1);
  const std::uint32_t c_m_norm_lo =
    (c_m_norm_radix_corrected >> h_m_bit_half_count);
  ...
  const std::uint32_t is_c_m_hidden_underflow_msb =
    (~is_c_m_hidden_nunderflow_msb);
  const std::uint32_t is_c_m_hidden_underflow =
    (((std::int32_t)is_c_m_hidden_underflow_msb) >> 31);
  const std::uint32_t c_m_hidden_underflow_normalized_sa =
    (c_m_norm_lo_nlz >> one);
  const std::uint32_t c_m_hidden_underflow_normalized =
    (c_m_norm_radix_corrected << c_m_hidden_underflow_normalized_sa);
  const std::uint32_t c_m_hidden_normalized =
    (c_m_norm_radix_corrected << c_m_norm_lo_nlz);
  const std::uint32_t c_e_hidden_normalized =
    (c_e_norm_radix_corrected - c_m_norm_lo_nlz);
  const std::uint32_t c_e_hidden =
    (c_e_hidden_normalized & ~is_c_m_hidden_underflow);
  const std::uint32_t c_m_hidden = half_private::_uint32_sels(
    is_c_m_hidden_underflow_msb, c_m_hidden_underflow_normalized,
    c_m_hidden_normalized);
  ...
  const std::uint32_t c_m_norm_rounded =
    (c_m_normalized + c_m_norm_round_amount);
  const std::uint32_t is_round_overflow_test =
    (c_e_normalized & m_round_overflow_bit);
  ...
  const std::uint32_t is_c_nan_via_a_msb =
    (is_a_e_flagged_msb & ~is_b_e_flagged_msb);
  const std::uint32_t is_c_nan_via_b_msb =
    (is_b_e_flagged_msb & ~is_b_m_eqz_msb);
  ...
  const std::uint32_t is_c_nan_min_via_a_msb =
    (is_a_e_flagged_msb & is_b_eqz_msb);
  const std::uint32_t is_c_nan_min_via_b_msb =
    (is_b_e_flagged_msb & is_a_eqz_msb);
  const std::uint32_t is_c_nan_min_msb =
    (is_c_nan_min_via_a_msb | is_c_nan_min_via_b_msb);
  const std::uint32_t is_c_inf_msb = (is_a_e_flagged_msb | is_b_e_flagged_msb);
  const std::uint32_t is_overflow_msb =
    (is_round_overflow_msb | is_mul_overflow_msb);
...
constexpr inline std::uint16_t half_sub(std::uint16_t ha,
                                        std::uint16_t hb) noexcept {
...
struct float16 {
  ...
  constexpr auto ieee_() const noexcept -> _ieee_t {
    return std::bit_cast<_ieee_t>(bits_);
  }
};

template <typename T> constexpr inline float16 to_float16(T val) noexcept {
...
struct float32 {
  ...
  constexpr inline auto ieee_() const noexcept -> _ieee_t {
    return std::bit_cast<_ieee_t>(bits_);
  }

  constexpr inline auto float_() const noexcept -> float {
...
inline constexpr float16 float32_to_float16(float input) noexcept {
  float32 f32 = {};
  f32.bits_ = std::bit_cast<std::uint32_t>(input);
  float16 f16 = {};
...
struct float16_t {
  ...
  explicit constexpr inline float16_t(float other) noexcept :
    data_{float16_t_private::float32_to_float16(other)} {}
  explicit constexpr inline float16_t(double other) noexcept :
    data_{float16_t_private::float32_to_float16(static_cast<float>(other))} {}
  explicit constexpr inline float16_t(int other) noexcept :
    data_{float16_t_private::float32_to_float16(static_cast<float>(other))} {}
  explicit constexpr inline float16_t(std::uint16_t bits) noexcept :
    data_{bits} {}
  ...
  explicit constexpr inline operator std::uint16_t() const noexcept {
    return data_.bits_;
  }
  ...
  constexpr inline float16_t operator-() const noexcept {
    return float16_t{static_cast<std::uint16_t>((data_.bits_ & 0x7fff) |
                                                (data_.bits_ ^ 0x8000))};
  }
...
constexpr inline float16_t fp16_max_subnormal{
...
constexpr inline float16_t fp16_min_positive_subnormal{
  static_cast<std::uint16_t>(0x1)};
constexpr inline float16_t fp16_nan{static_cast<std::uint16_t>(0x7e00)};
constexpr inline float16_t fp16_infinity_negative{
  static_cast<std::uint16_t>(0xfc00)};
...
constexpr inline float16_t operator/(float16_t lhs, float16_t rhs) noexcept {
  return float16_t{float(lhs) / float(rhs)};
}
...
operator<<(std::basic_ostream<CharT, Traits> &os, float16_t const &f) {
...
operator>>(std::basic_istream<CharT, Traits> &is, float16_t &f) {
  bool __fail = true;
  float __v;
...
auto constexpr inline make_trinary_function = [](auto const &func) noexcept {
  return [func](float16_t f1, float16_t f2, float16_t f3) -> float16_t {
    return float16_t{func(float(f1), float(f2), float(f3))};
  };
};
...
constexpr inline auto fmod = float16_t_private::make_binary_function(
  [](float f1, float f2) { return std::fmod(f1, f2); });
...
constexpr inline auto fmax = float16_t_private::make_binary_function(
  [](float f1, float f2) { return std::fmax(f1, f2); });
...
constexpr inline auto fdim = float16_t_private::make_binary_function(
  [](float f1, float f2) { return std::fdim(f1, f2); });
constexpr inline auto lerp = float16_t_private::make_trinary_function(
  [](float f1, float f2, float f3) { return f1 + f3 * (f2 - f1); });
...
constexpr inline auto pow = float16_t_private::make_binary_function(
  [](float f1, float f2) { return std::pow(f1, f2); });
...
inline float16_t hypot(float16_t f1, float16_t f2) {
  return float16_t{std::hypot(float(f1), float(f2))};
}
...
constexpr inline auto nearbyint = float16_t_private::make_unary_function(
  [](float f) { return std::nearbyint(f); });
...
//  assoc_laguerre, asso_legendre, hermite, legendre, laguerre, sph_bessel,
